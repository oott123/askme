...
<div class="ui negative message">
  <p><?=__('提交失败，请重试一次；如果持续失败，请换个浏览器试试。');?></p>
</div>
<?php endif; ?>
<a name="ask"></a>
<h2 class="ui header"><?=__('向我提问！');?></h2>
<form class="ui form" id="ask-form" method="post" action=".">
  <div class="field">
    <textarea name="text" placeholder="<?=__('任何问题都可以！提问是完全匿名的。');?>"><?=htmlspecialchars(isset($_POST['text']) ? $_POST['text'] : '');?></textarea>
  </div>
  <div class="login">
  ...
  <button name="submit" type="submit" class="ui fluid black submit button"><?=__('提交问题');?></button>
</form>
