...
<?php foreach($question_list as $question): ?>
  <div class="item" data-id="<?=$question['id'];?>">
    <div class="right floated content action">
      <a class="small ui button" href="answerme.php?id=<?=intval($question['id']);?>"><?=_('回答');?></a>
    </div>
    <div class="content">
      <div class="answer-item-content"><?=nl2br(htmlspecialchars($question['question']));?></div>
      <div class="meta"><span class="ui grey text"><?=_('提问于');?> <?=date('Y-m-d H:i', $question['asked_at']);?></span></div>
    </div>
  </div>
<?php endforeach; ?>
</div>
...
<form class="ui form" method="post" action="answerme.php">
  ...
  <button name="submit" type="submit" class="ui fluid submit button"><?=_('登出');?></button>
</form>
